...
change_dim:
    ...
    #pragma HLS bind_storage variable=zeros_buffer type=ram_t2p impl=bram
    ...
    for (int i = 0; i < I_BOUND ; i++) {
        ...
        for(int j = 0; j < J_BOUND ; j++){
            ...
            for (int k = 0; k < BLOCK_SIZE ;k++){
                ...
                hls::print("==============");
                hls::print("weight index: %d", (i * (GROUP_SIZE * NF) + j * BLOCK_SIZE + k));
                hls::print("dequant_weight: %f", dequant_weight);
            }
        }
    }
}
...
void krnl_c_proj(float* qkv_res, int512_t* c_proj_qweight, int512_t*c_proj_zeros, float* c_proj_scale, float* c_proj_bias, int s, float* out, float* c_proj_weight_buf) {
...
#pragma HLS INTERFACE m_axi port = c_proj_weight_buf offset = slave bundle = gmem6
...
#pragma HLS DATAFLOW
    ...
    // for (int i = 0; i < DATA_SIZE_PROJ_WEIGHT; i++){
    //     hls::print("index: %d",i);
    //     hls::print("dequant_weight: %f", c_proj_weight_buf[i]);
    // }
}
}
