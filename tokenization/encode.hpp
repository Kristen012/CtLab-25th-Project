...
//    int count = 0;
    std::cout << "Loaded vocab size: " << encoder.size() << std::endl;
...
//    }
    ...
    merges_stream.close();
    std::cout << "Loaded merge ranks: " << merge_ranks.size() << std::endl;

//    count = 0;
//    for (const auto& pair : merge_ranks) {
//        std::cout << pair.first << " " << pair.second << std::endl;
//        if (++count >= 10) break;
//    }
...
//    std::cout << "end_iter" << end_iter << endl;
    while (iter != end_iter) {
...
std::vector<int> encode() {
    tokenizer_t tokenizer("/home/ywtang23/Data/tokenization/vocab.txt", "/home/ywtang23/Data/tokenization/merges.txt");
    std::ifstream input_file("/home/ywtang23/Data/input.txt", std::ios::in | std::ios::binary);
