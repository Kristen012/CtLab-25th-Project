...
#define MAX_DEPTH 32
#define X_SIZE MAX_DEPTH * fc_K0 >> 1
#define W_SIZE (fc_K0 * fc_N0) >> 1
#define B_SIZE fc_N0 >> 1
#define R_SIZE MAX_DEPTH * fc_N0 >> 1
    ...
    for (int i = 0; i < MAX_DEPTH; i++) {
        if (i < depth) {
            ...
            for (int k = 0; k < fc_K0; k += 2) {
                float x1, x2;
                unpack_floats(X[(i * fc_K0 + k) >> 1], x1, x2);
                for (int j = 0; j < fc_N0; j += 2) {
#pragma HLS PIPELINE II=1
#pragma HLS UNROLL factor=4
                    float c1, c2, w1, w2, w3, w4, d1, d2;
                    unpack_floats(result[(iN0 + j) >> 1], c1, c2);
                    unpack_floats(W[(k * fc_N0 + j) >> 1], w1, w2); // Unpack weights corresponding to j
                    unpack_floats(W[((k + 1) * fc_N0 + j) >> 1], w3, w4); // Unpack weights corresponding to j + 1
...
#pragma HLS INTERFACE m_axi port=x offset=slave bundle=gmem12 depth=4096 max_read_burst_length=256
#pragma HLS INTERFACE m_axi port=w offset=slave bundle=gmem12 depth=4096 max_read_burst_length=256
#pragma HLS INTERFACE m_axi port=b offset=slave bundle=gmem12 depth=4096 max_read_burst_length=256
#pragma HLS INTERFACE m_axi port=out offset=slave bundle=gmem13 depth=4096 max_read_burst_length=256
