...
#define proj_K0 3072
#define proj_N0 258
#define MAX_SIZE (proj_K0 * proj_N0)
#define MAX_DEPTH 32
#define X_SIZE MAX_DEPTH * proj_K0 >> 1
#define W_SIZE (proj_K0 * proj_N0) >> 1
#define B_SIZE proj_N0 >> 1
#define R_SIZE MAX_DEPTH * proj_N0 >> 1
...
#pragma HLS PIPELINE II=1
        float val1 = in[i];
        float val2 = in[i + 1];
        local_in[i >> 1] = pack_floats(val1, val2);
    }
}
...
init_result:
    for (int i = 0; i < MAX_DEPTH; i++) {
        if (i < depth) {
            int iN0 = i * proj_N0;
            for (int j = 0; j < proj_N0; j += 2) {
#pragma HLS PIPELINE II=1
#pragma HLS UNROLL factor=16
                float bias_val1, bias_val2;
                unpack_floats(B[j >> 1], bias_val1, bias_val2);
                result[(iN0 + j) >> 1] = pack_floats(bias_val1, bias_val2);
            }
        }
    }
...
execute_mul:
    for (int i = 0; i < MAX_DEPTH; i++) {
        if (i < depth) {
            int iN0 = i * proj_N0;
            for (int k = 0; k < proj_K0; k += 2) {
                float x1, x2;
                unpack_floats(X[(i * proj_K0 + k) >> 1], x1, x2);
                for (int j = 0; j < proj_N0; j += 2) {
#pragma HLS PIPELINE II=1
#pragma HLS UNROLL factor=4
                    float c1, c2, w1, w2, w3, w4, d1, d2;
                    unpack_floats(result[(iN0 + j) >> 1], c1, c2);
                    unpack_floats(W[(k * proj_N0 + j) >> 1], w1, w2); // Unpack weights corresponding to j
                    unpack_floats(W[((k + 1) * proj_N0 + j) >> 1], w3, w4); // Unpack weights corresponding to j + 1

                    d1 = c1 + x1 * w1 + x2 * w3;
                    d2 = c2 + x1 * w2 + x2 * w4;

                    result[(iN0 + j) >> 1] = pack_floats(d1, d2);
                }
            }
        }
    }
...
#pragma HLS PIPELINE II=1
        ...
        unpack_floats(local_out[i >> 1], val1, val2);
...
void krnl_MLP_proj_linear(float* x, float* w, float* b, float* out, int depth) {
#pragma HLS INTERFACE m_axi port=x offset=slave bundle=gmem14 depth=4096 max_read_burst_length=256
#pragma HLS INTERFACE m_axi port=w offset=slave bundle=gmem14 depth=4096 max_read_burst_length=256
#pragma HLS INTERFACE m_axi port=b offset=slave bundle=gmem14 depth=4096 max_read_burst_length=256
#pragma HLS INTERFACE m_axi port=out offset=slave bundle=gmem15 depth=4096 max_read_burst_length=256
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    ...
    packed_t local_x[X_SIZE];
    packed_t local_w[W_SIZE];
    packed_t local_b[B_SIZE];
    packed_t local_result[R_SIZE];
...
#pragma HLS bind_storage variable=local_x type=RAM_T2P impl=bram
...
//#pragma HLS DATAFLOW
    load_input(x, local_x, depth * proj_K0);
    load_input(w, local_w, proj_K0 * proj_N0);
    load_input(b, local_b, proj_N0);

    compute_mul(local_x, local_w, local_b, local_result, depth);

    store_result(out, local_result, depth * proj_N0);
}
