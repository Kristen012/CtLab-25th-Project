    ...
    for (int i = 0; i < MAX_DEPTH; i++) {
        if (i < depth) {
            ...
            for (int k = 0; k < proj_K0; k += 2) {
                float x1, x2;
                unpack_floats(X[(i * proj_K0 + k) / 2], x1, x2);
                for (int j = 0; j < proj_N0; j += 2) {
#pragma HLS PIPELINE II=1
#pragma HLS UNROLL factor=2
                    float c1, c2, w1, w2, w3, w4, d1, d2;
                    unpack_floats(result[(iN0 + j) / 2], c1, c2);
                    unpack_floats(W[(k * proj_N0 + j) / 2], w1, w2); // Unpack weights corresponding to j
                    unpack_floats(W[((k + 1) * proj_N0 + j) / 2], w3, w4); // Unpack weights corresponding to j + 1
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    ...
    packed_t local_x[MAX_DEPTH * proj_K0 / 2];
    packed_t local_w[(proj_K0 * proj_N0) / 2];
    packed_t local_b[proj_N0 / 2];
    packed_t local_result[MAX_DEPTH * proj_N0 / 2];
