    ...
    for (int i = 0; i < NX / GROUP_SIZE ; i++) {
        ...
        for(int j = 0; j < GROUP_SIZE * NF / BLOCK_SIZE ; j++){
            ...
            for (int k = 0; k < BLOCK_SIZE ;k++){
                ...
                output[i * (GROUP_SIZE * NF) + j * BLOCK_SIZE + k] = static_cast<float>((int4_weight - int4_zero) * (scale_buffer[(j * BLOCK_SIZE + k) % NF]));
            }
        }
    }
}
...
#pragma HLS INTERFACE m_axi port = qweight bundle = gmem0 depth = (96*2304)
#pragma HLS INTERFACE m_axi port = qzeros bundle = gmem1 depth = (6*288)
#pragma HLS INTERFACE m_axi port = scale bundle = gmem2 depth = (6*2304)
